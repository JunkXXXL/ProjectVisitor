...
from observability import get_metrics_logger, measure_time
...
class RagFlowConnector(BDConnector):
    def __init__(self):
        ...
        self.logger = get_metrics_logger("database")

    def __enter__(self):
        try:
            with measure_time(self.logger, "database_response_time", operation="connect", database="rag_flow"):
    ...
    def add_knowledgebase(self, knowledgebase_name: str) -> Knowledgebase:
        with measure_time(self.logger, "database_response_time", operation="add_knowledgebase",
    ...
    def add_document(self, document_path: Path, kb_name: str) -> str:
        with measure_time(self.logger, "database_response_time", operation="add_document",
                          knowledgebase_name=kb_name, document_path=str(document_path)):
            with self.cnx.cursor() as cursor:
    ...
    def update_document(self, document_path: Path) -> None:
        with measure_time(self.logger, "database_response_time", operation="update_document",
    ...
    def is_knowledgebase_exist(self, kb_name: str) -> (bool, None | Knowledgebase):
        with measure_time(self.logger, "database_response_time", operation="is_knowledgebase_exist",
    ...
    def is_document_exist(self, kb_name: str, doc_name: Path) -> (bool, None | Document):
        with measure_time(self.logger, "database_response_time", operation="is_document_exist",
                          knowledgebase_name=kb_name, document_path=str(doc_name)):
            with self.cnx.cursor() as cursor:
                kb = KnowledgebaseService.get_by_name(cursor, kb_name)
